...
NBaseShader::NBaseShader() :
	mProgramShader_ID(0), mVertexShader_ID(0), mFragmentShader_ID(0),
	mNumAttributes(0)
{
}
...
void NBaseShader::LoadAndCompile(const std::string& vertexShaderFilePath,
	...
	CompileShader(fragmentFilePath, mFragmentShader_ID);
}
...
void NBaseShader::CompileShader(const std::string& filePath, GLuint shader_ID) {
	...
	if (shaderFile.fail()) {
		FatalError("Nose pudo abrir el archivo " + filePath);
	}
...
void NBaseShader::LinkShaders() {
	...
	if (isLinked == GL_FALSE) {
		GLint maxLength = 0;
		glGetShaderiv(mProgramShader_ID, GL_INFO_LOG_LENGTH, &maxLength);

		std::vector<char> errorLog(maxLength);
		glGetShaderInfoLog(mProgramShader_ID, maxLength, &maxLength, &errorLog[0]);
