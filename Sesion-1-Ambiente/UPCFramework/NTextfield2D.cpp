...
void NTextfield2D::Initialize(float x, float y, const std::string fontPath)
{
	...
	NContentManagement* contentManager = framework->GetContentManagement();

	mFontContent = contentManager->LoadAndGetContent<NFontContent>(fontPath);
	...
	const std::string shaderPath = "Content/Shaders/FontShader";

	NShaderManagement* shdMng = framework->GetShaderManagment();
	mShader = (NBaseShader*)shdMng->LoadAndGetShader<NFontShader>(shaderPath);
...
void NTextfield2D::AlignText(EnumTextAlign align) {
	...
	Vector2 textSizeTemp = Vector2::Zero;
	textSizeTemp.x = (mTextSize.x * GetScaleX()) * -1;
	textSizeTemp.x = (mTextSize.y * GetScaleY()) * -1;
...
void NTextfield2D::Update(float dt)
{
	...
	if (mDrawEnable && (mNeedMatrixUpdate || mIsTransformed))
	{
		glm::mat4 result(1.0f);
		glm::vec3 translate(mX + mTextInternalOffset, mY, 0.0f);
		glm::vec3 scale(mScaleX, mScaleY, 0.0f);
		glm::vec3 rotation(0.0f, 0.0f, 1.0f);

		result = glm::translate(result, translate);
		result = glm::scale(result, scale);
		result = glm::rotate(result, mRotationZ, rotation);
