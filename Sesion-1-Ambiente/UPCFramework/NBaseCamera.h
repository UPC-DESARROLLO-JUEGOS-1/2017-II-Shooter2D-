...
public :
	NBaseCamera() :
		mResultMatrix(1.0f), mWorldMatrix(1.0f), mViewMatrix(1.0f),
		mProjectionMatrix(1.0f), mNeedMatrixUpdate(true),
		mX(0), mY(0), mZ(0),
	...
	void SetX(float value) {
		if (mX != value) {
			...
			*(worldMatrixPtr + 12) = value; // position: 12 que seria: Position X
	...
	void SetY(float value) {
		if (mY != value) {
			...
			float* worldMatrixPtr = glm::value_ptr(mWorldMatrix);
			*(worldMatrixPtr + 13) = value; // position: 13 que seria: Position Y
	...
	void SetZ(float value) {
		if (mZ != value) {
			...
			float* worldMatrixPtr = glm::value_ptr(mWorldMatrix);
			*(worldMatrixPtr + 14) = value; // position: 14 que seria: Position Z
